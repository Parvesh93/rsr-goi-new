...
@section('content')
    ...
    <form id="bulk-clc-form"
      method="POST"
      action="{{ route('admin.degree-clc.bulk-clc') }}"
      target="_blank"
      style="display: none;">

    @csrf

    <div id="bulk-clc-inputs"></div>
</form>
...
$(document).on('change', '.all_select', function () {
    ...
    $('.clc-row-checkbox').prop('checked', checked);

});


$(document).on('change', '.clc-row-checkbox', function () {

    var total = $('.clc-row-checkbox').length;
    var checked = $('.clc-row-checkbox:checked').length;
...
$('#bulk-clc-print').on('click', function () {

    var selectedIds = [];

    $('.clc-row-checkbox:checked').each(function () {
        selectedIds.push($(this).val());
    });

    if (selectedIds.length === 0) {
        alert('Please select at least one CLC.');
        return;
    }

    $('#bulk-clc-inputs').empty();

    $.each(selectedIds, function (index, id) {

        $('#bulk-clc-inputs').append(
            $('<input>', {
                type: 'hidden',
                name: 'ids[]',
                value: id
