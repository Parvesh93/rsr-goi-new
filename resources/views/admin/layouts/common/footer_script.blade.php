...
<!-- CKEditor5 Ecosystem Documentation -->
<script src="{{ asset('dashboard/ckeditor5/ckeditor.js') }}"></script>
<script src="{{ asset('dashboard/js/form-editor.js') }}"></script>
                        ...
                        format: {
                            body: function(data, row, column, node) {
                                if (typeof data === 'string' && data.indexOf('<') !== -1) {
                                    ...
                                    // <br> को comma + space में बदलें
                                    html.find('br').replaceWith(', ');

                                    // सभी span का text collect करें
                                    let values = html.find('span').map(function() {
                                        return $(this).text().trim();
                                    }).get();

                                    // अगर span नहीं है तो full text लें
                                    if (values.length === 0) {
                                        values = [html.text().trim()];
                                    }

                                    return values.join(', ');
                                }

                                // plain text cells के लिए
                        ...
                        format: {
                            body: function(data, row, column, node) {
                                if (typeof data === 'string' && data.indexOf('<') !== -1) {
                                    ...
                                    // पहले <br> को comma+space से replace करें
                                    html.find('br').replaceWith(', ');

                                    // सभी <span> से text निकालें
                                    let values = html.find('span').map(function() {
                                        return $(this).text().trim();
                                    }).get();

                                    // अगर span नहीं है, तो पूरी HTML का text लें
                                    if (values.length === 0) {
                                        values = [html.text().trim()];
                                    }
                                ...
                                // Plain text या number cells के लिए
                                return (data !== null && data !== undefined) ? data.toString()
                                    .trim() : '';
                            }
                        }
                        ...
                        format: {
                            body: function(data, row, column, node) {
                                if (typeof data === 'string' && data.indexOf('<') !== -1) {
                                    ...
                                    // span text collect करें, अगर span नहीं है तो div/text भी ले लें
                                    let values = html.find('span').map(function() {
                                        return $(this).text().trim();
                                    }).get();

                                    // अगर span नहीं मिला तो पूरी HTML का plain text ले लें
                                    if (values.length === 0) {
                                        values = [html.text().trim()];
                                    }
...
@if ($version->direction == 1)
    <script type="text/javascript">
        ...
        tinymce.init({
            ...
            toolbar: 'insertfile undo redo | styles | bold italic | alignleft aligncenter alignright alignjustify | outdent indent | image',
