                ...
                <div class="col-sm-12">
                    <div class="card">
                        ...
                        <div class="card-block">
                            ...
                            <a href="{{ route($route.'.edit', $row->id) }}" class="btn btn-info"><i class="fas fa-sync-alt"></i>
                                {{ __('btn_refresh') }}</a>
                        </div>
                        ...
                        <div class="">
                            ...
                            <form id="" class="" action="{{ route($route . '.update',$row->id) }}" method="post">
                                @csrf
                                @method('PUT')
                                    ...
                                    <div class="col-md-12">
                                        <fieldset class="row scheduler-border">
                                             ...
                                             <div class="form-group col-md-6">
                                                <label for="mother_tongue">{{__('field_serial_no') }} <span>*</span></label>
                                                <input type="text" class="form-control" name="mr_no" id="mr_no"
                                                    value="{{ $row->mr_no }}" required>

                                                <div class="invalid-feedback">
                                                    {{ __('required_field') }} {{ __('field_serial_no') }}
                                                </div>
                                            </div>
                                            ...
                                            <div class="form-group col-md-6">
                                                <label for="registration_no">{{ __('field_regis_uni_no') }} <span>*</span></label>
                                                <input type="text" class="form-control" name="registration_no" id="registration_no"
                                                    value="{{ $row->registration_no }}" required>
                                            ...
                                            @if (isset($programs))
                                                @foreach ($programs->sortBy('title') as $program)
                                                    <option value="{{ $program->id }}"  @if ($row->program_id == $program->id) selected @endif
                                                        >
                                                ...
                                                <select class="form-control session" name="session" id="session">
                                                    ...
                                                    @if (isset($sessions))
                                                        @foreach ($sessions->sortByDesc('id') as $session)
                                                            <option value="{{ $session->id }}" @if ($row->session_id == $session->id) selected @endif
                                                                >
                                            ...
                                            <div class="form-group col-md-6">
                                                ...
                                                <select class="form-control " name="division" id="division" required>
                                                    <option value="">{{ __('select') }}</option>
                                                    <option value={{$row->division }} @if ($row->division == "FIRST") selected @endif>{{ "First Division" }}  </option>
                                                    <option value={{$row->division }} @if ($row->division == "SECOND") selected @endif>{{ "Second Division" }} </option>
                                                    <option value={{$row->division }} @if ($row->division == "THIRD") selected @endif>{{ "Third Division" }} </option>
                                                    <option value={{$row->division }} @if ($row->division == "FOURTH") selected @endif>{{ "Fourth Division" }} </option>
