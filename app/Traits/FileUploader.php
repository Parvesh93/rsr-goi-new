...
trait FileUploader {
    ...
    public function uploadImage(Request $request, $attach, $directory, $width, $height) {
        ...
        if($request->hasFile($attach)){
            ...
            $img = ImageManager::imagick()->read($request->file($attach)->getRealPath())->resize($width, $height, function ($constraint) { $constraint->aspectRatio(); $constraint->upsize(); })->save($thumbnailpath);
