...
use App\Models\InterClc;
use App\Models\Program;
use App\Models\Session;
use Illuminate\Http\Request;

class InterClcController extends Controller
{
    ...
    {
        // Module Data
        $this->title = trans_choice('module_inter_clc', 1);
        $this->route = 'admin.inter-clc';
        $this->view = 'admin.inter-clc';
        $this->path = 'inter-clc';
        $this->access = 'inter-clc';
     ...
     */
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            if (isset($request->session) || isset($request->program)  || isset($request->student_id) || isset($request->student_regi)) {
                ...
                $inter_clc = InterClc::where('status', '1');

                if (!empty($request->program)) {
                    $inter_clc->where('program_id', $program);
                }
                if (!empty($request->session)) {
                    $inter_clc->where('session_id', $session);
                }
                if (!empty($request->student_id)) {
                    $inter_clc->where('student_id', 'like', '%' . $student_id . '%');
                }
                if (!empty($request->student_regi)) {
                    $inter_clc->where('registration_no', 'like', '%' . $student_regi . '%');
                }




                $data['rows'] = $inter_clc->orderby('id', 'desc')->get();
            }
   ...
   public function create()
    ...
    public function store(Request $request)
    {
        ...
        try {
            //Data Insert
            $inter_clc = new InterClc();
            $inter_clc->student_id = $request->student_id;
            $inter_clc->registration_no = $request->registration_no;
            
            $inter_clc->batch_id = $request->batch;
            $inter_clc->program_id = $request->program;

            $inter_clc->session_id = $request->session;
            $inter_clc->name = $request->name;
            $inter_clc->father_name = $request->father_name;
            $inter_clc->mother_name = $request->mother_name;
            $inter_clc->division = $request->division;
            $inter_clc->date = $request->dob;
        
            $inter_clc->college_name = $request->college_name;
           
            $inter_clc->save();
            $inter_clc->serial_no = '000' . $inter_clc->id;
     ...
     */
    public function edit(Request $request, InterClc $inter_clc)
    {
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            $data['row'] = $inter_clc;
            $teacherDepa = auth()->user()->teacher_department; // ya ->faculty_name
            $program_id = $inter_clc->program_id; // ya ->faculty_name
            $collegeDepartment = auth()->user()->college_department_id;
     ...
     */
    public function update(Request $request, InterClc $inter_clc)
    {
     ...
     */
    public function destroy(InterClc $inter_clc)
    {
     ...
     public function clc($id)
    {
        ...
        $data['rows'] = InterClc::where('id', $id)->orderBy('student_id', 'asc')->get();
