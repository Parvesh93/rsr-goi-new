...
class OldStudentController extends Controller
{
    ...
    {
        // Module Data
        $this->title = trans_choice('module_pharmacy_clc', 1);
        $this->route = 'admin.old-student';
        $this->view = 'admin.old-student';
        $this->path = 'old-student';
        $this->access = 'old-student';
    ...
    public function index(Request $request)
    {
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            if (isset($request->session) || isset($request->program)  || isset($request->student_id) || isset($request->student_regi)) {
                ...
                $old_students = OldStudent::where('status', '1');

                if (!empty($request->program)) {
                    $old_students->where('program_id', $program);
                }
                if (!empty($request->session)) {
                    $old_students->where('session_id', $session);
                }
                if (!empty($request->student_id)) {
                    $old_students->where('student_id', 'like', '%' . $student_id . '%');
                }
                if (!empty($request->student_regi)) {
                    $old_students->where('registration_no', 'like', '%' . $student_regi . '%');
                }




                $data['rows'] = $old_students->orderby('id', 'desc')->get();
            }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $old_student->student_id = $request->student_id;
            $old_student->registration_no = $request->registration_no;
            
            $old_student->batch_id = $request->batch;
            $old_student->program_id = $request->program;

            $old_student->session_id = $request->session;
            $old_student->name = $request->name;
            $old_student->father_name = $request->father_name;
            $old_student->mother_name = $request->mother_name;
            $old_student->division = $request->division;
            $old_student->date = $request->dob;
            

            $old_student->college_name = $request->college_name;
            $old_student->status = '1';
            $old_student->save();
            
            $old_student->clc_no = '0' . $old_student->id;
            $old_student->save();
     ...
     */
    public function edit(Request $request, OldStudent $old_student)
    {
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            $data['row'] = $old_student;
            $teacherDepa = auth()->user()->teacher_department; // ya ->faculty_name
            $program_id = $old_student->program_id; // ya ->faculty_name
            $collegeDepartment = auth()->user()->college_department_id;
     ...
     */
    public function update(Request $request, OldStudent $old_student)
    {
     ...
     */
    public function destroy(OldStudent $old_student)
    {
    ...
    public function clc($id)
    {
        ...
        $data['rows'] = OldStudent::where('id', $id)->orderBy('student_id', 'asc')->get();
