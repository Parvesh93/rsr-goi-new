   ...
   */
   public function index()
   {
      ...
      $data['library_books'] = Book::where('status', '1')->get();
          ...
          $data['pending_applications'] = Application::where('status', '1')->where('program_id',$teacherDepa)->get();
          $data['active_students'] = Student::where('status', '1')->where('program_id',$teacherDepa)->get();
          
          
          $data['active_staffs'] = User::where('college_department_id',$collegeDepartment)->where('is_admin',0)->where('status', '1')->get();
     ...
     public function pendingStudent()
   {
       ...
       if(auth()->user()->is_admin === 1){
        ...
        //   dd()
          ...
          $data['title'] = $this->title;
          $data['route'] = 'admin.application';
          $data['view'] = $this->view;
          $data['pending_applications'] = Application::where('status', '1')->get();
        
          return view($this->view.'.pending_student', $data);
       }elseif(auth()->user()->is_admin === 0){
   ...
   public function activeStudent()
   {
        ...
        if(auth()->user()->is_admin === 1)
        {
           ...
           $data['title'] = $this->title;
           $data['route'] = 'admin.student';
          
          $data['access'] = 'student';
          $data['view'] = $this->view;
           $data['active_students'] = Student::where('status', '1')->get();
       
           return view($this->view.'.active_student', $data);
