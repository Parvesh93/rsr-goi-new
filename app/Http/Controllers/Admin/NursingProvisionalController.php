...
use App\Models\NursingProvisional;
use App\Models\Program;
use App\Models\Session;
use Illuminate\Http\Request;

class NursingProvisionalController extends Controller
{
    /**
    ...
    {
        // Module Data
        $this->title = trans_choice('module_nursing_provisional', 1);
        $this->route = 'admin.nursing-provisional';
        $this->view = 'admin.nursing-provisional';
        $this->path = 'nursing-provisional';
        $this->access = 'nursing-provisional';
    ...
    public function index(Request $request)
    {
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            if (isset($request->session) || isset($request->program)  || isset($request->student_id) || isset($request->student_regi)) {
                ...
                $nursing_provisional = NursingProvisional::where('status', '1');

                if (!empty($request->program)) {
                    $nursing_provisional->where('program_id', $program);
                }
                if (!empty($request->session)) {
                    $nursing_provisional->where('session_id', $session);
                }
                if (!empty($request->student_id)) {
                    $nursing_provisional->where('student_id', 'like', '%' . $student_id . '%');
                }
                if (!empty($request->student_regi)) {
                    $nursing_provisional->where('registration_no', 'like', '%' . $student_regi . '%');
                }




                $data['rows'] = $nursing_provisional->orderby('id', 'desc')->get();
            }
    ...
    public function store(Request $request)
    {
        ...
        try {
            //Data Insert
            $nursing_provisional = new NursingProvisional();
            $nursing_provisional->student_id = $request->student_id;
            $nursing_provisional->registration_no = $request->registration_no;

            $nursing_provisional->batch_id = $request->batch;
            $nursing_provisional->program_id = $request->program;

            $nursing_provisional->session_id = $request->session;
            $nursing_provisional->name = $request->name;
            $nursing_provisional->father_name = $request->father_name;
            $nursing_provisional->mother_name = $request->mother_name;
            $nursing_provisional->result = $request->result;
            $nursing_provisional->date = $request->dob;

            $nursing_provisional->save();
            $nursing_provisional->serial_no = '000' . $nursing_provisional->id;
     ...
     */
    public function edit(Request $request, NursingProvisional $nursing_provisional)
    {
        // dd($id);
        // dd($request->all());
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            $data['row'] = $nursing_provisional;
            $teacherDepa = auth()->user()->teacher_department; // ya ->faculty_name
            $program_id = $nursing_provisional->program_id; // ya ->faculty_name
            $collegeDepartment = auth()->user()->college_department_id;
     ...
     */
    public function update(Request $request, NursingProvisional $nursing_provisional)
    {
     ...
     */
    public function destroy(NursingProvisional $nursing_provisional)
    {
        //Delete Data
        $nursing_provisional->delete();
    ...
    public function provisional($id)
    {
        ...
        $data['rows'] = NursingProvisional::where('id', $id)->orderBy('student_id', 'asc')->get();
