...
use App\Models\NursingClc;
use App\Models\Program;
use App\Models\Session;
use Illuminate\Http\Request;

class NursingClcController extends Controller
{
    ...
    {
        // Module Data
        $this->title = trans_choice('module_nursing_clc', 1);
        $this->route = 'admin.nursing-clc';
        $this->view = 'admin.nursing-clc';
        $this->path = 'nursing-clc';
        $this->access = 'nursing-clc';
     ...
     */
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            if (isset($request->session) || isset($request->program)  || isset($request->student_id) || isset($request->student_regi)) {
                ...
                $nursing_clc = NursingClc::where('status', '1');

                if (!empty($request->program)) {
                    $nursing_clc->where('program_id', $program);
                }
                if (!empty($request->session)) {
                    $nursing_clc->where('session_id', $session);
                }
                if (!empty($request->student_id)) {
                    $nursing_clc->where('student_id', 'like', '%' . $student_id . '%');
                }
                if (!empty($request->student_regi)) {
                    $nursing_clc->where('registration_no', 'like', '%' . $student_regi . '%');
                }




                $data['rows'] = $nursing_clc->orderby('id', 'desc')->get();
            }
   ...
   public function create()
    ...
    public function store(Request $request)
    {
        ...
        try {
            //Data Insert
            $nursing_clc = new NursingClc();
            $nursing_clc->student_id = $request->student_id;
            $nursing_clc->registration_no = $request->registration_no;
            
            $nursing_clc->batch_id = $request->batch;
            $nursing_clc->program_id = $request->program;

            $nursing_clc->session_id = $request->session;
            $nursing_clc->name = $request->name;
            $nursing_clc->father_name = $request->father_name;
            $nursing_clc->mother_name = $request->mother_name;
            $nursing_clc->division = $request->division;
            $nursing_clc->date = $request->dob;
        
            $nursing_clc->college_name = $request->college_name;
           
            $nursing_clc->save();
            $nursing_clc->serial_no = '000' . $nursing_clc->id;
     ...
     */
    public function edit(Request $request, NursingClc $nursing_clc)
    {
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            $data['row'] = $nursing_clc;
            $teacherDepa = auth()->user()->teacher_department; // ya ->faculty_name
            $program_id = $nursing_clc->program_id; // ya ->faculty_name
            $collegeDepartment = auth()->user()->college_department_id;
     ...
     */
    public function update(Request $request, NursingClc $nursing_clc)
    {
     ...
     */
    public function destroy(NursingClc $nursing_clc)
    {
     ...
     public function clc($id)
    {
        ...
        $data['rows'] = NursingClc::where('id', $id)->orderBy('student_id', 'asc')->get();
