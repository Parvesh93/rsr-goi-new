...
use App\Models\EngineeringClc;
use App\Models\Faculty;
use App\Models\Program;
use App\Models\Session;
use Illuminate\Http\Request;

class EngineeringClcController extends Controller
{
    ...
    {
        // Module Data
        $this->title = trans_choice('module_engineering_clc', 1);
        $this->route = 'admin.engineering-clc';
        $this->view = 'admin.engineering-clc';
        $this->path = 'engineering-clc';
        $this->access = 'engineering-clc';
     ...
     public function index(Request $request)
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            if (isset($request->session) || isset($request->program)  || isset($request->student_id) || isset($request->student_regi)) {
                ...
                $engineering_clc = EngineeringClc::where('status', '1');

                if (!empty($request->program)) {
                    $engineering_clc->where('program_id', $program);
                }
                if (!empty($request->session)) {
                    $engineering_clc->where('session_id', $session);
                }
                if (!empty($request->student_id)) {
                    $engineering_clc->where('student_id', 'like', '%' . $student_id . '%');
                }
                if (!empty($request->student_regi)) {
                    $engineering_clc->where('registration_no', 'like', '%' . $student_regi . '%');
                }




                $data['rows'] = $engineering_clc->orderby('id', 'desc')->get();
            }
   ...
   public function create()
    ...
    public function store(Request $request)
    {
        ...
        try {
            //Data Insert
            $engineering_clc = new EngineeringClc();
            $engineering_clc->student_id = $request->student_id;
            $engineering_clc->registration_no = $request->registration_no;
            
            $engineering_clc->batch_id = $request->batch;
            $engineering_clc->program_id = $request->program;

            $engineering_clc->session_id = $request->session;
            $engineering_clc->name = $request->name;
            $engineering_clc->father_name = $request->father_name;
            $engineering_clc->mother_name = $request->mother_name;
            $engineering_clc->division = $request->division;
            $engineering_clc->date = $request->dob;
        
            $engineering_clc->college_name = $request->college_name;
           
            $engineering_clc->save();
            $engineering_clc->serial_no = '000' . $engineering_clc->id;
     ...
     */
    public function edit(Request $request, EngineeringClc $engineering_clc)
    {
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            $data['row'] = $engineering_clc;
            $teacherDepa = auth()->user()->teacher_department; // ya ->faculty_name
            $program_id = $engineering_clc->program_id; // ya ->faculty_name
            $collegeDepartment = auth()->user()->college_department_id;
     ...
     */
    public function update(Request $request, EngineeringClc $engineering_clc)
    {
     ...
     */
    public function destroy(EngineeringClc $engineering_clc)
    {


        //Delete Data
        $engineering_clc->delete();
     ...
     public function clc($id)
    {
        ...
        $data['rows'] = EngineeringClc::where('id', $id)->orderBy('student_id', 'asc')->get();
