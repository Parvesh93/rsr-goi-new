...
class ProvisionalController extends Controller
{
    ...
    {
        // Module Data
        $this->title = trans_choice('module_provisional', 1);
        $this->route = 'admin.provisional';
        $this->view = 'admin.provisional';
        $this->path = 'provisional';
        $this->access = 'provisional';
    ...
    public function index(Request $request)
    {
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            if (isset($request->session) || isset($request->program)  || isset($request->student_id) || isset($request->student_regi)) {
                ...
                $old_students = Provisional::where('status', '1');
                ...
                $data['provisionals'] = $old_students->orderby('student_id', 'desc')->get();
                
                //  $rows = $old_students->orderby('id', 'desc')->get();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $provisional->student_id = $request->student_id;
            $provisional->registration_no = $request->registration_no;
            
            $provisional->batch_id = $request->batch;
            $provisional->program_id = $request->program;

            $provisional->session_id = $request->session;
            $provisional->name = $request->name;
            $provisional->father_name = $request->father_name;
            $provisional->mother_name = $request->mother_name;
            $provisional->result = $request->result;
            $provisional->date = $request->dob;
            $provisional->passing_date = $request->passing_date;
            $provisional->status = '1';
            $provisional->save();
            $provisional->provisional_serial_no = '000' . $provisional->id;
     ...
     */
    public function edit(Request $request,Provisional $provisional)
    {
         // dd($id);
        // dd($request->all());
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            $data['row'] = $provisional;
            $teacherDepa = auth()->user()->teacher_department; // ya ->faculty_name
            $program_id = $provisional->program_id; // ya ->faculty_name
            $collegeDepartment = auth()->user()->college_department_id;
     ...
     */
    public function update(Request $request, Provisional $provisional)
    {
    ...
    //   dd('s');
        $data['rows'] = Provisional::where('id', $id)->orderBy('student_id', 'asc')->get();
