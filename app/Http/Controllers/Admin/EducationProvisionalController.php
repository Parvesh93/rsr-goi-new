...
class EducationProvisionalController extends Controller
{
     /**
    ...
    {
        // Module Data
        $this->title = trans_choice('module_education_provisional', 1);
        $this->route = 'admin.education-provisional';
        $this->view = 'admin.education-provisional';
        $this->path = 'education-provisional';
        $this->access = 'education-provisional';
    ...
    public function index(Request $request)
    {
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            if (isset($request->session) || isset($request->program)  || isset($request->student_id) || isset($request->student_regi)) {
                ...
                $education_provisional = EducationProvisional::where('status', '1');

                if (!empty($request->program)) {
                    $education_provisional->where('program_id', $program);
                }
                if (!empty($request->session)) {
                    $education_provisional->where('session_id', $session);
                }
                if (!empty($request->student_id)) {
                    $education_provisional->where('student_id', 'like', '%' . $student_id . '%');
                }
                if (!empty($request->student_regi)) {
                    $education_provisional->where('registration_no', 'like', '%' . $student_regi . '%');
                }




                $data['rows'] = $education_provisional->orderby('id', 'desc')->get();
            }
    ...
    public function store(Request $request)
    {
        ...
        try {
            //Data Insert
            $education_provisional = new EducationProvisional();
            $education_provisional->student_id = $request->student_id;
            $education_provisional->registration_no = $request->registration_no;

            $education_provisional->batch_id = $request->batch;
            $education_provisional->program_id = $request->program;

            $education_provisional->session_id = $request->session;
            $education_provisional->name = $request->name;
            $education_provisional->father_name = $request->father_name;
            $education_provisional->mother_name = $request->mother_name;
            $education_provisional->result = $request->result;
            $education_provisional->date = $request->dob;

            $education_provisional->save();
            $education_provisional->serial_no = '000' . $education_provisional->id;
     ...
     */
    public function edit(Request $request, EducationProvisional $education_provisional)
    {
        // dd($id);
        // dd($request->all());
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            $data['batches'] = Batch::where('status', '1')->orderBy('id', 'desc')->get();
            $data['row'] = $education_provisional;
            $data['programs'] = Program::where('status', '1')
                ->orderBy('title', 'asc')
                ->get();
            $data['sessions'] = Session::where('status', '1')->orderBy('title', 'asc')->get();
            ...
            $data['row'] = $education_provisional;
            $teacherDepa = auth()->user()->teacher_department; // ya ->faculty_name
            $program_id = $education_provisional->program_id; // ya ->faculty_name
            $collegeDepartment = auth()->user()->college_department_id;
     ...
     */
    public function update(Request $request, EducationProvisional $education_provisional)
    {
     ...
     */
    public function destroy(EducationProvisional $education_provisional)
    {
        //Delete Data
        $education_provisional->delete();
    ...
    public function provisional($id)
    {
        ...
        $data['rows'] = EducationProvisional::where('id', $id)->orderBy('student_id', 'asc')->get();
