...
use App\Models\DegreeClc;
use App\Models\Faculty;
use App\Models\Program;
use App\Models\Session;
use Illuminate\Http\Request;

class DegreeClcController extends Controller
{
    ...
    {
        // Module Data
        $this->title = trans_choice('module_degree_clc', 1);
        $this->route = 'admin.degree-clc';
        $this->view = 'admin.degree-clc';
        $this->path = 'degree-clc';
        $this->access = 'degree-clc';
   ...
   public function index(Request $request)
    {
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            if (isset($request->session) || isset($request->program)  || isset($request->student_id) || isset($request->student_regi)) {
                ...
                $degree_clc = DegreeClc::where('status', '1');

                if (!empty($request->program)) {
                    $degree_clc->where('program_id', $program);
                }
                if (!empty($request->session)) {
                    $degree_clc->where('session_id', $session);
                }
                if (!empty($request->student_id)) {
                    $degree_clc->where('student_id', 'like', '%' . $student_id . '%');
                }
                if (!empty($request->student_regi)) {
                    $degree_clc->where('registration_no', 'like', '%' . $student_regi . '%');
                }




                $data['rows'] = $degree_clc->orderby('id', 'desc')->get();
            }
   ...
   public function create()
    ...
    public function store(Request $request)
    {
        ...
        try {
            //Data Insert
            $degree_clc = new DegreeClc();
            $degree_clc->student_id = $request->student_id;
            $degree_clc->registration_no = $request->registration_no;
            $degree_clc->exam_role_no = $request->exam_role_no;
            $degree_clc->mr_no = $request->mr_no;
            
            $degree_clc->batch_id = $request->batch;
            $degree_clc->program_id = $request->program;

            $degree_clc->session_id = $request->session;
            $degree_clc->name = $request->name;
            $degree_clc->father_name = $request->father_name;
            $degree_clc->mother_name = $request->mother_name;
            $degree_clc->division = $request->division;
            $degree_clc->date = $request->dob;
        
            $degree_clc->college_name = $request->college_name;
           
            $degree_clc->save();
            $degree_clc->serial_no = '000' . $degree_clc->id;
     ...
     */
    public function edit(Request $request, DegreeClc $degree_clc)
    {
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            $data['row'] = $degree_clc;
            $teacherDepa = auth()->user()->teacher_department; // ya ->faculty_name
            $program_id = $degree_clc->program_id; // ya ->faculty_name
            $collegeDepartment = auth()->user()->college_department_id;
     ...
     */
    public function update(Request $request, DegreeClc $degree_clc)
    {
     ...
     */
    public function destroy(DegreeClc $degree_clc)
    {
     ...
     public function clc($id)
    {
        ...
        $data['rows'] = DegreeClc::where('id', $id)->orderBy('student_id', 'asc')->get();

        return view($this->view . '.clc', $data);
    }
    
    
    public function bulkClc(Request $request)
{
    $request->validate([
    ...
    $data['rows'] = DegreeClc::whereIn('id', $request->ids)
